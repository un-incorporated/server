...
|------|-------------|--------------|
| **HashiCorp Packer** | Bakes the per-role disk images (`uninc-proxy`, `uninc-db`, `uninc-observer`) on every release tag. Each image carries Docker, every container image (`proxy`, `observer`, `nats`, `pgbouncer`, `caddy`, `minio`), and the static compose YAML. | Standard tool for the "build a VM image once, boot many VMs from it" pattern. The `qemu` builder boots a Debian 12 cloud image, runs the install script, snapshots the disk, and emits a portable `disk.raw`. Artifact is published to GitHub Releases — same place as the WASM verifier, free distribution, no cloud-vendor registry. Customers' GCP projects lazy-import on first deploy of each release tag. Sibling to our Docker build pipeline — different artifact, same shape. |
